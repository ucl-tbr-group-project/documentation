...
in Section~\ref{sec:supervised}, we first gather a sufficiently large
...
The adaptive sampling approach that we characterise in Section~\ref{sec:adaptive} can be viewed as a more general problem.
...
Table~\ref{tbl:metrics} provides an exhaustive listing and description of metrics recorded
...
optimise against metrics described in Section~\ref{sec:metrics}. To attain meaningful
and practically usable results, we require a sufficiently large and diverse pool
of surrogate families to review. This is described by the listing in Table~\ref{tbl:surrogates}. The presented selection of models includes basic
...
(illustrated in Figure~\ref{fig:nn-archs}). This allows us to realise a simplistic
