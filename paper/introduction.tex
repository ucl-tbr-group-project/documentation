...
ratio (TBR) in a Inertial confinement fusion (ICF) nuclear fusion reactor. The
expensive model (which our surrogate model approximates) is a Monte Carlo (MC)
neutronics simulation, \textit{Paramak}\footnote{Provided by collaborator
Jonathan Shimwell, at UKAEA.}\todo{IOP guidelines discourage footnotes}, which returns a prediction of the TBR for a given configuration of a spherical (ICF) reactor.
...
into two classes. Continuous parameters, such as material thicknesses and
packing fraction, describe the geometry of a given reactor configuration.
\todo{add all the parameters here} Discrete
categorical parameters specify material selections, such as
