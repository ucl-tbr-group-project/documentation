...
In this work, we develop an empirical
surrogate model for the tritium breeding ratio
(TBR) in an inertial confinement fusion (ICF)
reactor. The expensive model which our
surrogate model approximates is a Monte
Carlo (MC) neutronics simulation, \textit{Paramak} which returns a prediction of the TBR for a given configuration of a spherical
ICF reactor. This was achieved by containerizing (using Docker) the neutronics workflow and providing a (python flask based) API interface that returned TBR values for the provided inputs. The neutronics workflow used OpenMC, Fendl 3.1d nuclear data and a Python 3 environment for receiving the API inputs and for driving the model creation, simulation and results extraction.
...
reactor, whose input parameters (exhaustively listed in~\Tref{tbl:params}) in the expensive MC model fall
into 2~classes. While the geometry of a given reactor is described by continuous
parameters, material selections are specified by discrete
categorical parameters. We set out to produce a fast TBR
...
\begin{table}[t]
	...
	\caption{\label{tbl:params}Input parameters supplied to the MC TBR simulation in alphabetical
